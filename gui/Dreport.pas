...
const
  ...
  Categories: String = 'Nodes'#13'Links';
...
begin
  ...
  begin
    ...
    XCategoryCombo.Items.Text := Categories;
    XCategoryCombo.ItemIndex := 0;
    YCategoryCombo.Items.Text := Categories;
    YCategoryCombo.ItemIndex := 1;
...
// VariableCombo control depending on whether Nodes, Links
...
begin
  ...
  begin
    ...
    2: begin
         ...
         for I := Uglobals.SYSOUTVAR1 to Uglobals.SYSOUTVAR2 do
...
begin
  ...
  case theCategory of
  0: if not Project.IsNode(CurrentList) then
     begin
       Uutils.MsgDlg(MSG_NO_NODE_SELECTED, mtError, [mbOK]);
       Exit;
     end;
  1: if not Project.IsLink(CurrentList) then
     begin
...
//-----------------------------------------------------------------------------
begin
  if ItemIndex = 0 then Result := NODES
  else if ItemIndex = 1 then Result := LINKS
  else Result := SYS;
end;
