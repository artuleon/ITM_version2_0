...
begin
  ...
  if nToks < 2 then
    ...
    if (CtrlType = 2) and (nToks < 7) then Result := ErrMsg(ITEMS_ERR, '');
  end;

  if Result = 0 then
  begin
    aLink.Data[CtrlIndex]   := TokList[1];
    aLink.Data[CtrlIndex+1] := TokList[2];
    aLink.Data[CtrlIndex+2] := TokList[3];
    if CtrlType = 1 then
    begin
      aLink.Data[CtrlIndex+3] := TokList[4];
    end
    else begin
      aLink.Data[CtrlIndex+3] := '';
      aLink.Data[CtrlIndex+4] := TokList[5];
      aLink.Data[CtrlIndex+5] := TokList[6];
    end;
  end;
end;
