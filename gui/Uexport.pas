...
begin
  ...
  Hdr1 := Hdr1 + 'Pump            ' + Tab + 'Piping  ' + Tab;
  Hdr2 := Hdr2 + 'Curve           ' + Tab + 'Diam.   ' + Tab;
  Hdr3 := Hdr3 + '----------------' + Tab + '--------' + Tab;

  Hdr1 := Hdr1 + 'Piping  ' + Tab + 'Frict.  ' + Tab + 'Loss    ';
  Hdr2 := Hdr2 + 'Length  ' + Tab + 'Factor  ' + Tab + 'Coeff.  ';
  Hdr3 := Hdr3 + '--------' + Tab + '--------' + Tab + '--------';
  ...
  with Project.Lists[PUMP] do
    ...
    begin
      ...
      S.Add(Line);
      if not SameText(L.Data[PUMP_CONTROL_INDEX], 'NONE') then Inc(CtrlCount);
    end;
end;
...
begin
  ...
  Hdr1 := Hdr1 + 'Orifice ' + Tab + 'Orifice ' + Tab + 'Inlet   ' + Tab + 'Disch.  ' + Tab + 'Flap    ';
  Hdr2 := Hdr1 + 'Type    ' + Tab + 'Shape   ' + Tab + 'Offset  ' + Tab + 'Coeff.  ' + Tab + 'Gate    ';
  Hdr3 := Hdr1 + '--------' + Tab + '--------' + Tab + '--------' + Tab + '--------' + Tab + '--------';
  ...
  with Project.Lists[ORIFICE] do
    ...
    begin
      ...
      S.Add(Line);
      if not SameText(L.Data[ORIFICE_CONTROL_INDEX], 'NONE') then Inc(CtrlCount);
    end;
end;
...
begin
  ...
  Hdr1 := Hdr1 + 'End     ' + Tab + 'Can     ';
  Hdr2 := Hdr2 + 'Cons.   ' + Tab + 'Surchg. ';
  Hdr3 := Hdr3 + '--------' + Tab + '--------';
...
//      Line := Line + Tab + Format('%-8s', [L.Data[WEIR_END_COEFF_INDEX]]);
      ...
      S.Add(Line);
      if not SameText(L.Data[WEIR_CONTROL_INDEX], 'NONE') then Inc(CtrlCount);
    end;
end;
...
begin
  ...
  Hdr1 := ';;              ' + Tab + 'Init.   ' + Tab + 'Time to ' + Tab + 'Control ' + Tab;
  Hdr2 := ';;Link          ' + Tab + 'Setting ' + Tab + 'Close   ' + Tab + 'Type    ' + Tab;
  Hdr3 := ';;--------------' + Tab + '--------' + Tab + '--------' + Tab + '--------' + Tab;

  Hdr1 := Hdr1 + 'Control         ' + Tab + 'Control         ' + Tab + 'Control         ';
  Hdr2 := Hdr1 + 'Series          ' + Tab + 'Node            ' + Tab + 'Curve           ';
  Hdr3 := Hdr1 + '----------------' + Tab + '----------------' + Tab + '----------------';
  ...
  begin
    ...
    begin
      ...
      Line := Format('%-16s', [L.Data[ID_INDEX]]);
      Line := Line + Tab + Format('%-8s', [L.Data[CtrlIndex + CTRL_INIT_SETTING_INDEX]]);
      Line := Line + Tab + Format('%-8s', [L.Data[CtrlIndex + CTRL_CLOSE_RATE_INDEX]]);
      Line := Line + Tab + Format('%-8s', [L.Data[CtrlIndex]]);
      if SameText(L.Data[CtrlIndex], 'TIME') then
        Line := Line + Tab + Format('%-16s', [L.Data[CtrlIndex + CTRL_TIME_SERIES_INDEX]])
      else begin
        Line := Line + Tab + '*               ';
        Line := Line + Tab + Format('%-16s', [L.Data[CtrlIndex + CTRL_CONTROL_NODE_INDEX]]);
        Line := Line + Tab + Format('%-16s', [L.Data[CtrlIndex + CTRL_CONTROL_CURVE_INDEX]]);
      end;
      S.Add(Line);
    end;
  end;
