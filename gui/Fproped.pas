...
begin
  ...
  // User wants to edit a Pump's pump curve or controls
  else if (CurrentList = PUMP) then
  begin
    if Index = PUMP_CURVE_INDEX then EditCurve(PUMPCURVE, S)
    else if Index = PUMP_CONTROL_INDEX then
      Uedit.EditControl(PUMP, Project.CurrentItem[CurrentList], S, Modified);
  end

  // User wants to edit an Orifice's controls
  else if (CurrentList = ORIFICE) and (Index = ORIFICE_CONTROL_INDEX)
  then Uedit.EditControl(ORIFICE, Project.CurrentItem[CurrentList], S, Modified)

  // User wants to edit a Weir's controls
  else if (CurrentList = WEIR) and (Index = WEIR_CONTROL_INDEX)
  then Uedit.EditControl(WEIR, Project.CurrentItem[CurrentList], S, Modified)
