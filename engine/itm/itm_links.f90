...
real(8) :: ag, d, f, k_piping, k_local
    ...
    k_local = pumps(pump_index)%loss_coeff
    k = k_piping + k_local
...
real(8) c_orif, c_weir, f, head
    ...
    ! Find control setting applied to orifice & effective discharge coeff.
    call apply_control(orifs(i)%control, T_GLOBAL, DT_GLOBAL)
    call orifice_coeffs(link_index, c_orif, c_weir)
    
    ! Find head across orifice & degree that opening is full (f)
...
real(8) :: h1, h2, hcrest, hcrown, hmidpt
   ...
   ! Compute elevations of orifice crest and crown
    ...
    ! Compute fraction of orifice opening filled by inlet water level
    f = 1d0
...
real(8) c_weir, c_orif, f, hcrest, hcrown, head, h1, h2
    ...
    ! Find control setting applied to weir & effective discharge coeff.
    call apply_control(weirs(i)%control, T_GLOBAL, DT_GLOBAL)
 
   ! Compute elevations of weir crest and crown
    j1 = Node1(link_index)
    ...
    ! Find heads on upstream side (h1) & downstream side (h2) of weir
    call weir_heads(link_index, h1, h2, direction)
    
    ! If weir is surcharged then treat it as an orifice
    if (h1 >= hcrown) then
...
subroutine weir_heads(link_index, h1, h2, direction)
!====================================================================
! Finds heads on either side of weir.
!====================================================================
integer, intent(in ) :: link_index
...
real(8) :: y, head, c_orif
    ...
    ! Find head seen by equivalent orifice
    y = (hcrest + hcrown) / 2d0
    if ( h2 < y ) then
        head = h1 - y
    else
    ...
    ! Find flow coeff. for equivalent orifice
    y = hcrown - hcrest
    c_orif = normal_weir_flow(link_index, y) / sqrt(y/2d0)
    
    ! Compute flow through equivalent orifice
...
integer :: j
    ...
    if (BCnode(j) .eq. RESERVOIR) then
        ...
        if (BCnode(j) .eq. DROPSHAFT .or. BCnode(j) .eq. JUNC2) then
            new_depth = yres_jun_old(j) + link_flow*DT_GLOBAL/Ares_junct(j)
            if (new_depth > hdrops_overf(j)) then
                link_flow = (hdrops_overf(j) - yres_jun_old(j)) * &
                    Ares_junct(j) / DT_GLOBAL
            end if
        end if
    end if
...
real(8) :: new_depth, min_depth, old_volume, new_volume
    ...
    min_depth = 0.30
    call itm_get_storage(node_index, yres_jun_old(node_index), old_volume)
    ...
    if (new_depth <= min_depth) then
        call itm_get_storage(node_index, min_depth, new_volume)
...
real(8) :: new_depth, min_depth, old_volume, new_volume
    ...
    if (new_depth > reser_maxdepth(node_index)) then
        call itm_get_storage(node_index, reser_maxdepth(node_index), new_volume)
