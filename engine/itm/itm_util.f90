...
!Version:      2.0
!Module:       itm_util
!Description:  computes critical and normal flow depth
!Authors:      see AUTHORS
...
!	External Crit_iter	
	...
	if (Qc <= 0d0 .or. Qc > Qcrit_maxIA(j))then 
		write(98,*),'Subroutine Ycrit. Qc is < 0 or extremely large'
		write(99,*),'Subroutine Ycrit. Qc is < 0 or extremely large'
	...
	If(conver_result == 1)then	
		write(98,*),'Subr. Util. No conv. critical flow'
          write(99,*),'Subr. Util. No conv. critical flow'
...
!	External Norm_iter
	...
	if (Q <= 0d0 .or. Q >1.d16)then 
          write(98,*),'Subroutine Ynormal. Q is <= 0 or extremely large'
          write(99,*),'Subroutine Ynormal. Q is <= 0 or extremely large'
	...
	call converg (conver_result, info)	
	If(conver_result == 1)then	
          write(98,*),'Subr. Util, Norm_iter'
          write(99,*),'Subr. Util, Norm_iter'          
		!write(98,*),'Q =',Q
  !        write(98,*),'Slope', S0(j)
...
!	External Conjug_iter
	...
	if (Q <= 0.000001*Qmin(j) .or. Q >1.d20)then 
		write(98,*),'Yconjugate. Q is < 0 or extremely large'
          write(99,*),'Yconjugate. Q is < 0 or extremely large'
		call endprog; GLOBAL_STATUS_FLAG = 1; return
	endif	
	parint1000 = 0
	...
	if (x(1) <= 0.01*b2_min(j))then
      ...
      If (ISNAN(x(1)))then !To make sure x(1) is not a NAN
          ...
          write(99,*),'NaN is found in Subr. Util. Conjugate depth' 
          call endprog; GLOBAL_STATUS_FLAG = 1; return
	endif
